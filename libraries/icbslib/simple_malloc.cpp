 ...
 * @LastEditTime: 2020-09-28 11:34:16
 * @LastEditors: kay
 */
#include <memory>
...
namespace icbs {   
   struct dsmalloc {
      ...
      char* operator()(size_t sz, uint8_t align_amt=8) {
         ...
         icbs::check(GROW_MEMORY(pages_to_alloc) != -1, "failed to allocate pages");  
         return ret;
      }
...
void* malloc(size_t size) {
   void* ret = icbs::_dsmalloc(size);
   return ret;
}

void* memset(void*,int,size_t);
void* calloc(size_t count, size_t size) {
   if (void* ptr = icbs::_dsmalloc(count*size)) {
