...
#include "core/icbs/powers.hpp"
#include "contracts/icbs/system.hpp"
#include "contracts/icbs/privileged.hpp"
...
namespace icbs {
   ...
   bool block_signing_authority_v0::is_valid()const {
      ...
      std::set<icbs::public_key> unique_keys;
   ...
   // privileged.hpp
   void set_blockchain_parameters(const icbs::blockchain_parameters& params) {
      char buf[sizeof(icbs::blockchain_parameters)];
      icbs::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(icbs::blockchain_parameters& params) {
      char buf[sizeof(icbs::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      icbs::check( size <= sizeof(buf), "buffer is too small" );
      icbs::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }

   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = icbs::pack( prods );
