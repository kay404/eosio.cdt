...
namespace icbs {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         icbs::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         icbs::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         icbs::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         icbs::check( -max_amount <= amount, "subtraction underflow" );
         icbs::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         icbs::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         icbs::check( -max_amount <= amount, "addition underflow" );
         icbs::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         icbs::check( tmp <= max_amount, "multiplication overflow" );
         icbs::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         icbs::check( a != 0, "divide by zero" );
         icbs::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         icbs::check( b.amount != 0, "divide by zero" );
         icbs::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         icbs::check( a.contract == b.contract, "type mismatch" );
