...
namespace icbs {
   ...
   icbs::public_key recover_key( const icbs::checksum256& digest, const icbs::signature& sig ) {
      ...
      icbs::public_key pubkey;
      if ( pubkey_size <= sizeof(optimistic_pubkey_data) ) {
         icbs::datastream<const char*> pubkey_ds( optimistic_pubkey_data, pubkey_size );
         pubkey_ds >> pubkey;
      } else {
         ...
         icbs::datastream<const char*> pubkey_ds( reinterpret_cast<const char*>(pubkey_data), pubkey_size );
         pubkey_ds >> pubkey;
   ...
   void assert_recover_key( const icbs::checksum256& digest, const icbs::signature& sig, const icbs::public_key& pubkey ) {
      ...
      auto sig_data = icbs::pack(sig);
      auto pubkey_data = icbs::pack(pubkey);
