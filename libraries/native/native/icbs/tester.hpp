...
inline bool expect_assert(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   if (ret == 0) {
      ...
      if (!check)
         icbs::check(false, std::string("error : expect_assert, no assert {"+li+"}").c_str());
      icbs::print("error : expect_assert, no assert {"+li+"}\n");
   ...
   silence_output(false);
   if (!check)
      icbs::check(passed, std::string("error : expect_assert, wrong assert {"+li+"}").c_str());
   if (!passed)
      icbs::print("error : expect_assert, wrong assert {"+li+"}\n");
   silence_output(disable_out);
...
inline bool expect_print(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   silence_output(false);
   if (!check)
      icbs::check(passed, std::string("error : wrong print message {"+li+"}").c_str());
   if (!passed)
      icbs::print("error : wrong print message {"+li+"}\n");
   silence_output(disable_out);
   return passed;
}
...
#define CHECK_EQUAL(X, Y) \
   if (!(X == Y)) { \
      ___has_failed = true; \
      icbs::print(std::string("CHECK_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}\n"); \
   }

#define REQUIRE_EQUAL(X, Y) \
   icbs::check(X == Y, std::string(std::string("REQUIRE_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}").c_str());
...
#define EOSIO_TEST(X) \
   ...
   if ( X ## _ret == 0 ) \
      ...
      silence_output(false); \
      icbs::print("\033[1;37m", #X, " \033[0;37munit test \033[1;31mfailed\033[0m (aborted)\n"); \
      ___has_failed = true; \
